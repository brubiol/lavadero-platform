...
class WeatherAdjustedForecasterTest {
    ...
    void should_recover_negative_rain_coefficient_when_rain_correlates_with_drops() {
        // Series: ~50 cars baseline, but rain days lose 20 cars per mm of precip.
        ...
        WeatherAugmentedModel model = WeatherAdjustedForecaster.fit(history, weather);
    ...
    void should_fall_back_to_seasonal_when_weather_features_are_constant() {
        // Constant weather → OLS has no signal to fit; precip beta should be ~0
        // and predictions should match the bare seasonal forecaster's predictions.
        LocalDate start = LocalDate.of(2025, 1, 6);
        List<HistoricalPoint> history = new ArrayList<>();
        List<WeatherFeatures> weather = new ArrayList<>();
        ...
        WeatherAugmentedModel adjusted = WeatherAdjustedForecaster.fit(history, weather);
        List<ForecastPoint> withWeather = WeatherAdjustedForecaster.predict(
                adjusted, start.plusDays(125), 7,
                List.of(
                        new WeatherFeatures(start.plusDays(125), 0.0, 28.0, 0.0),
                        new WeatherFeatures(start.plusDays(126), 0.0, 28.0, 0.0),
                        new WeatherFeatures(start.plusDays(127), 0.0, 28.0, 0.0),
                        new WeatherFeatures(start.plusDays(128), 0.0, 28.0, 0.0),
                        new WeatherFeatures(start.plusDays(129), 0.0, 28.0, 0.0),
                        new WeatherFeatures(start.plusDays(130), 0.0, 28.0, 0.0),
                        new WeatherFeatures(start.plusDays(131), 0.0, 28.0, 0.0)));
    ...
    void should_apply_forecast_weather_to_horizon_predictions() {
        // Train on a 180-day window where rain depresses demand, then verify the
        // forecast for a rainy horizon day comes in noticeably lower than a dry one.
        LocalDate start = LocalDate.of(2025, 1, 6);
        ...
        List<WeatherFeatures> dryForecast = List.of(
                new WeatherFeatures(horizonStart, 0.0, 28.0, 0.0));
        List<WeatherFeatures> rainyForecast = List.of(
                new WeatherFeatures(horizonStart, 10.0, 28.0, 0.0));

        double dry = WeatherAdjustedForecaster.predict(model, horizonStart, 1, dryForecast).get(0).predicted();
        double rainy = WeatherAdjustedForecaster.predict(model, horizonStart, 1, rainyForecast).get(0).predicted();
